...
export default function StripePaymentSuccess() {
  ...
  useEffect(() => {
    ...
    const activateUserPackage = async () => {
      try {
        setActivating(true);

        // 1. Confirm that payment was successful
        console.log("✅ Confirming payment for session:", sessionId);
        await paymentsApi.getPaymentSuccess(sessionId);

        // 2. Get the pending package details saved from PaymentModal
        const pendingStr = localStorage.getItem("pendingPackage");
        if (!pendingStr) {
          throw new Error("No pending package found in localStorage");
        }
        ...
        // 🔥 3. THIS IS WHAT YOU WANT → Call POST /user-package
        const userPackageDto = {
          role: pending.roleId, // Note: "role" not "roleId"
          startDate: now,
          months: pending.months,
        ...
        console.log("✅ User Package created successfully:", packageResponse);

        // 4. Refresh active package for UI
        const latestPackage = await userPackageApi.getActivePackage();
        setActivePackage(latestPackage);
        ...
        console.error("❌ Failed to activate user package:", err);

        setError(
          err.response?.data?.message ||
            "Payment was successful, but we couldn't activate your package. Please contact support.",
        );
      } finally {
        setActivating(false);
        setLoading(false);
