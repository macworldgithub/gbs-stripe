  ...
  const handlePaymentSuccess = async () => {
    try {
      ...
      try {
        ...
        console.log('User package not available yet, might need webhook processing');
      }
    } catch (err) {
      console.error('Failed to process payment success:', err);
      setError('Payment was successful but we had trouble updating your account. Please contact support.');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-6">
        ...
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gbs-text-primary mb-2">Processing Your Payment...</h2>
          <p className="text-gbs-text-muted">Please wait while we confirm your subscription.</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-6 max-w-md mx-auto">
        ...
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gbs-text-primary mb-2">Payment Issue</h2>
    ...
    <div className="flex flex-col items-center justify-center min-h-[400px] gap-6 max-w-md mx-auto">
      ...
      <div className="text-center animate-fade-in">
        ...
        <p className="text-gbs-text-muted mb-6">
          {paymentData?.message || 'Your subscription has been activated successfully.'}
        </p>

        {sessionId && (
          <div className="bg-gbs-bg-card border border-gbs-border rounded-xl p-4 mb-6 text-left">
            <h3 className="text-sm font-semibold text-gbs-text-primary mb-3">Transaction Details</h3>
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gbs-text-secondary">{new Date().toLocaleDateString()}</span>
              </div>
            </div>
          </div>
        )}
