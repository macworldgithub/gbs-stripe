...
export default function PaymentModal({
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (isRole) {
        // ====================== UPGRADE FLOW ======================
        // Prepare pending data to be processed AFTER successful payment on the Success page
        const pendingData = {
          roleId: planId,
          months,
          trial: false,
        ...
        // Redirect to Stripe Update Checkout
        console.log("🔄 Redirecting to Stripe Update Checkout...");
        await stripeService.redirectToUpdateCheckout({
          role: planId,
          months,
        });
      ...
      console.error("Payment initiation failed:", err);
      setError(err.response?.data?.message || "Failed to process payment redirection.");
    } finally {
      setProcessing(false);
    }
  };
