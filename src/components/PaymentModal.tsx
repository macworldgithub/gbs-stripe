...
export default function PaymentModal({
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // months is a whole number: 1 for monthly, 12 for yearly
      const months = billingCycle === "monthly" ? 1 : 12;
      const now = new Date().toISOString();
  ...
  // basePrice in DB is the annual price.
  // Monthly display = basePrice / 12 (per month equivalent)
  // Yearly display  = basePrice (full annual price)
  const finalPrice =
    billingCycle === "monthly"
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="flex bg-gray-100 rounded-2xl p-1 mb-10">
                <button
                  onClick={() => setBillingCycle("monthly")}
                  className={`flex-1 py-3.5 rounded-xl font-medium ${billingCycle === "monthly"
                    ? "bg-white shadow text-gray-900"
                    : "text-gray-600"
                    }`}
                >
                ...
                <button
                  onClick={() => setBillingCycle("yearly")}
                  className={`flex-1 py-3.5 rounded-xl font-medium ${billingCycle === "yearly"
                    ? "bg-white shadow text-gray-900"
                    : "text-gray-600"
                    }`}
                >
